...
x = params.x; %256*256
blocksize = params.blocksize;%8*8
trainnum = params.trainnum;%40000
dictsize = params.dictsize;%256
...
% blocksize %
if (numel(blocksize)==1)%numel: 返回数组A中元素个数
...
if (isfield(params,'maxval'))
    maxval = params.maxval;%强度为255
%Specifies the range of the signal values. When
% 'maxval' is specified, this range changes to [0,maxval].
else
    maxval = 1;
...
if (isfield(params,'initdict') && ischar(params.initdict))
    if (strcmpi(params.initdict,'odct'))
        params.initdict = odctndict(blocksize,dictsize,p);%64*256
        %初始化DCT字典
...
%parms.sigma =8
params.Edata = sqrt(prod(blocksize)) * sigma * gain;%73.6   % target error for omp
...
%生成测试数据  params.data is 64*40000
